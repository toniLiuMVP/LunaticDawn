...
def build_readme() -> str:
    ...
    lines.append("| 俠客遊 II 深度心得 + 多項勘誤（2023 PTT Old-Games）★ | **kyleul**（凱布洛 / 阿里不達） |")
    ...
    lines.append("| 第三之書工具開發（2011 跨海寄送） | daggerain（對岸玩家） |")
    lines.append("| 前途道標角色複製人物存檔包 | Cody |")
    lines.append("")
    lines.append("### 跨海玩家回報（17-19 年前的真實問題佐證 — 本站 2026 字體修復補丁誕生的緣由）")
    lines.append("")
    lines.append("- **tyler_yeung**（香港玩家）— 2007 前途道標 XP 字型問題（19 年前最早佐證）")
    lines.append("- **cu54102003**（家伶）— 2008 XP 字型問題再回報")
    lines.append("- **杜偉祥**（towaicheung，香港玩家）— 2009 前途道標 XP 字體重疊（17 年前佐證）")
    lines.append("- **Tedk**（對岸玩家）— 2009 Google Pages 時代協助驗證")
    ...
    lines.append("- **QQ508091419**（對岸玩家）— 2020 俠客遊 IV 神之眼路徑詢問")
    lines.append("- **DavidChien / nec81616 / tigerzhou100800 / line841a** — 2020-2021 Google 防毒誤判回報")
    lines.append("- **qq888999123**（廖姓玩家）— 2021「密碼壓縮繞防毒」解法提議")
    lines.append("- **a0961168526** — 2022 阿米卓古 / 第三之書 me 版詢問")
    lines.append("- **raymond0206**（香港玩家）— 2022 Win10 亂碼問題")
    lines.append("- **jill90131** — 2023 第三之書入坑詢問（toni TeamViewer 遠端協助）")
    lines.append("- **KO**（compound_angles，香港玩家）— 2023-2024 跨年度多次回報 + 乙太劍研究實測")
    lines.append("- **kyleul**（凱布洛）— 2007 PTT XP 字型問題 + 2023 PTT 深度心得")
